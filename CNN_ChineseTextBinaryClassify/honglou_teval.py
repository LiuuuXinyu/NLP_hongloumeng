...
tf.flags.DEFINE_string("positive_data_file1", '.\\fenci\\real_data_test.txt', "Data source for the positive data.")
tf.flags.DEFINE_string("negative_data_file1",'.\\fenci\\bullshit_data_test.txt' , "Data source for the positive data.")


# Eval Parameters
tf.flags.DEFINE_integer("batch_size1", 64, "Batch Size (default: 64)")
# 填写训练获得模型的存储位置
tf.flags.DEFINE_string("checkpoint_dir1", ".\\runs\\1594054688\\checkpoints\\", "Checkpoint directory from training run")
tf.flags.DEFINE_boolean("eval_train1", True, "Evaluate on all training data")

# Misc Parameters
tf.flags.DEFINE_boolean("allow_soft_placement1", True, "Allow device soft device placement")
tf.flags.DEFINE_boolean("log_device_placement1", False, "Log placement of ops on devices")
...
def eval_test(real_data_path,bullshit_data_path,relative_path):
    ...
    FLAGS.positive_data_file1 = real_data_path
    FLAGS.negative_data_file1 = bullshit_data_path
    FLAGS.checkpoint_dir1 = relative_path
    #print(FLAGS)
    # CHANGE THIS: Load data. Load your own data here
    if FLAGS.eval_train1:
        #print(FLAGS.eval_train)
        #print(FLAGS.positive_data_file)
        #print(FLAGS.negative_data_file)
        #x_raw, y_test = data_helpers.load_data_and_labels(FLAGS.positive_data_file, FLAGS.negative_data_file)

        x_raw = data_helpers.load_single_file(FLAGS.positive_data_file1)
    ...
    vocab_path = os.path.join(FLAGS.checkpoint_dir1, "..", "vocab")
    ...
    # Evaluation
    # ==================================================
    checkpoint_file = tf.train.latest_checkpoint(FLAGS.checkpoint_dir1)
    print("checkpoint_file========", checkpoint_file)

    graph = tf.Graph()
    with graph.as_default():
        session_conf = tf.ConfigProto(
            allow_soft_placement=FLAGS.allow_soft_placement1,
            log_device_placement=FLAGS.log_device_placement1)
        ...
        with sess.as_default():
            ...
            batches = data_helpers.batch_iter(list(x_test), FLAGS.batch_size1, 1, shuffle=False)
...
def all_test(data_path,mode2):
    ...
    #mode2 = 1
    if mode2 == 0:
        file_out_dir = '.\\hongloumeng_chap\\chapter\\'
        data_out_dir = "./honglou_cnn_train_file/"
        endlen = sum([len(x) for _, _, x in os.walk(os.path.dirname(file_out_dir))])

    elif mode2 == 1:
        file_out_dir = '.\\hongloumeng_chap\\chapter2\\'
        data_out_dir = ".\\honglou_cnn_test_file\\"
        endlen = sum([len(x) for _, _, x in os.walk(os.path.dirname(file_out_dir))])
...
def main():
    ...
    relative_test_path = "./runs/1593940145/checkpoints/"
    all_test(relative_test_path,1)
