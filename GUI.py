	...
	def ClickTestAllButton(self, event):
		...
		try:
			...
			if self.radiobox2.GetSelection() == 0:
				...
				if self.FileName_8.GetValue().split('\\')[-1] == '1594125564': 
					for i in range(1,28):
						...
						relative_test_path = ".\\CNN_ChineseTextBinaryClassify\\runs\\1594125564\\checkpoints\\"
				...
				elif self.FileName_8.GetValue().split('\\')[-1] == '1594128385': 
					for i in range(1,20):


						real_test_data_path = ".\\CNN_ChineseTextBinaryClassify\\honglou_cnn_test_file\\chap" +str(i)+".txt"
						bullshit_test_data_path = ".\\CNN_ChineseTextBinaryClassify\\fenci\\bullshit_data_test.txt"
						relative_test_path = ".\\CNN_ChineseTextBinaryClassify\\runs\\1594128385\\checkpoints\\"
	...
	def ClickTestOneButton(self, event):
		...
		try:
			if self.radiobox2.GetSelection() == 0:
				...
				if self.FileName_8.GetValue().split('\\')[-1] == '1594125564': #Bullshit的单回问题
				...
				elif self.FileName_8.GetValue().split('\\')[-1] == '1594128385': #红楼梦的单回问题
			...
			if self.radiobox2.GetSelection() != 2:
				...
				elif self.FileName_8.GetValue().split('\\')[-1] == '1594128385':#HLM的单回问题
